...
class DDiTBlock(nn.Module):
    ...
    def forward(self, x, rotary_cos_sin, c, seqlens=None, causal=False):
...
class SEDD(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(self, indices, sigma, causal=False):
        ...
        with torch.amp.autocast("cuda", dtype=torch.bfloat16):
            for i in range(len(self.blocks)):
                x = self.blocks[i](x, rotary_cos_sin, c, seqlens=None, causal=causal)
